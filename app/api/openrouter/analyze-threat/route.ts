  ...
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return NextResponse.json(
        { success: false, error: 'OpenRouter API key not configured' } as AnalyzeThreatResponse,
        { status: 500 }
      );
    }

    const body: AnalyzeThreatRequest = await request.json();
    
    if (!body.freeformText || typeof body.freeformText !== 'string') {
      return NextResponse.json(
        { success: false, error: 'freeformText is required' } as AnalyzeThreatResponse,
        { status: 400 }
      );
    }
