...
interface FreeFormAbnormalityInputProps {
  value: string;
  onChange: (value: string) => void;
  onStructuredDataChange: (data: StructuredAbnormalityData) => void;
  onSuggestionsChange: (suggestions: AISuggestion[]) => void;
  privacyLevel: 'high' | 'medium' | 'low';
}

export default function FreeFormAbnormalityInput({
  value,
  onChange,
  ...
  const analyzeInput = useCallback(async (text: string) => {
    ...
    try {
      // Analyze the input
      const analyzeResponse = await fetch('/api/openrouter/analyze-abnormality', {
        method: 'POST',
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
          Describe the abnormality or situation
        </label>
        <button
          type="button"
      ...
      <div className="relative">
        <textarea
          ...
          placeholder="Describe the abnormality or situation in your own words... 
