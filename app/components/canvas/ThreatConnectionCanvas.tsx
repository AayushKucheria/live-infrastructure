...
interface ThreatConnectionCanvasProps {
  mainThreat: ThreatBubbleUnion;
  initialRelevantThreats: ThreatBubbleUnion[];
  libraryThreats: ThreatBubbleUnion[];
  onAnalyze: (source: ThreatBubbleUnion, target: ThreatBubbleUnion) => Promise<string>;
}

interface CanvasNode {
  id: string;
  type: 'main' | 'magic' | 'threat';
  x: number;
  y: number;
  data?: ThreatBubbleUnion;
}
...
export default function ThreatConnectionCanvas({
  mainThreat,
  initialRelevantThreats,
  libraryThreats: initialLibraryThreats,
  onAnalyze
}: ThreatConnectionCanvasProps) {
  const router = useRouter();
  // --- State ---
  const [libraryThreats, setLibraryThreats] = useState<ThreatBubbleUnion[]>(initialLibraryThreats);
  ...
  useEffect(() => {
    ...
    const initialNodes: CanvasNode[] = [
      { id: 'main-threat', type: 'main', x: centerX - 600, y: centerY - 200, data: mainThreat },
      { id: 'magic-node', type: 'magic', x: centerX, y: centerY },
    ];

    // Position initial relevant threats
    initialRelevantThreats.forEach((threat, index) => {
      const angle = (index - (initialRelevantThreats.length - 1) / 2) * 0.6; // Spread in radians
      const radius = 500;
      initialNodes.push({
        id: threat.id,
        type: 'threat',
        x: centerX + 500, // Base X offset
        y: centerY + (index - (initialRelevantThreats.length - 1) / 2) * 350, // Vertical stack spread
        data: threat,
      });
    });
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over.id === 'canvas-area') {
      const libraryThreat = libraryThreats.find(t => t.id === draggedId);
      if (libraryThreat) {
        ...
        setLibraryThreats(prev => prev.filter(t => t.id !== draggedId));
        setSelectedNodeId(libraryThreat.id); // Auto select newly dropped node

        // Analyze
        try {
          const explanation = await onAnalyze(mainThreat, libraryThreat);
          setExplanations(prev => ({ ...prev, [libraryThreat.id]: explanation }));
        } catch (error) {
  ...
  const handleNodeClick = (id: string) => {
      ...
      // If clicking main threat, maybe clear selection since it's special?
      if (id === 'main-threat') {
  ...
  const activeLibraryThreat = libraryThreats.find(t => t.id === activeId);
  ...
  const getThreatNodes = () => nodes.filter(n => n.type === 'threat');
  ...
  return (
    <div className="h-full w-full relative overflow-hidden bg-zinc-900">
      ...
      >
            ...
            <div onClick={handleCanvasClick}>
                <CanvasArea>
                      ...
                      <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible z-0">
                        ...
                        {magicNode && threatNodes.map(node => (
                             <ConnectionLine
                                key={`conn-${node.id}`}
                      ...
                      {nodes.map((node) => (
                        <div
                          ...
                          onClick={(e) => {
                              e.stopPropagation();
                              if (node.type === 'threat') handleNodeClick(node.id);
                          }}
                        >
                          ...
                          {node.type === 'threat' && node.data && (
                             <DraggableThreatCard
                               ...
                               onCommunicate={() => router.push(`/threat/${mainThreat.id}/communicate?target=${node.id}`)}
                             />
                          )}
                        </div>
                      ))}
        ...
        <FloatingDock threats={libraryThreats} />
        ...
        <DragOverlay>
          {activeLibraryThreat ? (
             <div className="w-[280px] rotate-3 cursor-grabbing opacity-90">
                <ThreatBubbleCard bubble={activeLibraryThreat} disableLink />
             </div>
          ) : activeCanvasNode && activeCanvasNode.data && activeCanvasNode.type === 'threat' ? (
