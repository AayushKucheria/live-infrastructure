// AI threat analysis utilities for freeform threat bubble creation

import { callAI, AIMessage } from './ai';

export interface StructuredThreatData {
  description: string;
  location: string;
...
/**
 * Analyzes freeform text input and extracts structured threat data
 */
export async function analyzeThreatInput(freeformText: string): Promise<StructuredThreatData> {
  const systemPrompt = `You are analyzing freeform text from a research lab about a potential biothreat detection. Extract structured information from the text.

Return a JSON object with these fields:
- description: A clear summary/description of the threat
...
If information is missing, use reasonable defaults or empty strings. Be concise and factual.`;
  ...
  const messages: AIMessage[] = [
    ...
    { role: 'user', content: `Extract structured data from this threat report:\n\n${freeformText}` }
  ];
  ...
  try {
    ...
    console.error('Error analyzing threat input:', error);
...
export async function organizeByPrivacyLevel(
  ...
  const systemPrompt = `You are organizing threat information into three privacy levels for sharing:
...
`;
  ...
  const messages: AIMessage[] = [
    { role: 'system', content: systemPrompt },
    { role: 'user', content: `Organize this threat data:\n\n${dataSummary}` }
  ];
...
export async function integrateClarifications(
  ...
  const systemPrompt = `You are integrating user clarifications into threat bubble data. Update the structured data with the clarifications provided. Return the updated JSON structure.`;
